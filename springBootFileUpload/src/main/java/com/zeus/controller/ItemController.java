...
public class ItemController {
	...
	public ResponseEntity<byte[]> displayFile(Item item) throws Exception {
		...
		try {
			...
			// IOUtils.toByteArray(in) inputstream에 저장된 파일을 바이트배열로 변환해 주는 것, 헤더에 정보를
			// 전달(headers)하고 생성(HttpStatus.CREATED)한다.
	...
	public String updateForm(Item i, Model model) throws Exception {
	...
	@GetMapping("/delete")
	public String delete(Item i, Model model) throws Exception {
		...
		model.addAttribute("message", "%s 상품 삭제가 실패되었습니다.".formatted(item.getName()));
		return "item/failed";
	}
